...
def plot_coordinate_systems(x, y, z, e, n, u, n_ned, e_ned, d):
    fig = plt.figure(figsize=(12, 10))

    # ECEF Plot
    ax1 = fig.add_subplot(221, projection='3d')
    ax1.scatter(x, y, z, color='b', s=100)
    ax1.set_xlabel('X (meters)')
    ax1.set_ylabel('Y (meters)')
    ax1.set_zlabel('Z (meters)')
    ax1.set_title('ECEF Coordinate System')
    ax1.text(x, y, z, f'Point: ({x:.2f}, {y:.2f}, {z:.2f})', color='black')

    # ENU Plot
    ax2 = fig.add_subplot(222, projection='3d')
    ax2.scatter(e, n, u, color='g', s=100)
    ax2.set_xlabel('East (meters)')
    ax2.set_ylabel('North (meters)')
    ax2.set_zlabel('Up (meters)')
    ax2.set_title('ENU Coordinate System')
    ax2.text(e, n, u, f'Point: ({e:.2f}, {n:.2f}, {u:.2f})', color='black')

    # NED Plot
    ax3 = fig.add_subplot(223, projection='3d')
    ax3.scatter(n_ned, e_ned, -d, color='r', s=100)
    ax3.set_xlabel('North (meters)')
    ax3.set_ylabel('East (meters)')
    ax3.set_zlabel('Down (meters)')
    ax3.set_title('NED Coordinate System')
    ax3.text(n_ned, e_ned, -d, f'Point: ({n_ned:.2f}, {e_ned:.2f}, {-d:.2f})', color='black')

    # LLA Plot
    ax4 = fig.add_subplot(224)
    ax4.axis('off')  # Hide axes for LLA
    ax4.text(0.5, 0.5, f'LLA Coordinates:\nLatitude: {latitude:.6f}\nLongitude: {longitude:.6f}\nAltitude: {altitude:.2f} m',
...
def main():
    ...
    # Convert ECEF to NED
    n_ned, e_ned, d = ecef_to_ned(x, y, z, ref_lat, ref_lon, ref_alt)
    print(f"NED: North: {n_ned:.2f} m, East: {e_ned:.2f} m, Down: {d:.2f} m")

    # Plot all coordinate systems in one figure
    plot_coordinate_systems(x, y, z, e, n, u, n_ned, e_ned, d)
